// `primitive_index` written as a per-primitive mesh shader output and read back in the fragment
// shader. Mesh shading enables SPIR-V's `PrimitiveId` on its own, so this must not need the
// geometry shader capability - see `primitive-index-mesh-shader.toml`.
